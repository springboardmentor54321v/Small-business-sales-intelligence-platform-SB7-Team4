...
def _local_churn_fallback(customer_id):
    """Local inference using trained Random Forest Churn model when remote API is unreachable."""
    try:
        ...
        if cust.empty:
            return {
                ...
                "Risk Factors": ["New customer account with no churn warning signs."],
                "Prescriptive Actions": ["Deliver welcome onboarding package with first-order incentive."]
            }
        ...
        if "Order Date" in cust.columns:
            dates = pd.to_datetime(cust["Order Date"], errors="coerce").dropna().sort_values()
        ...
        if churn_prob >= 0.70:
            ...
            actions = [
                ...
                "Enroll in loyalty rewards tier"
            ]
        else:
            risk = "Low Risk"
            factors = [
                f"Healthy repeat activity ({freq} orders, ₹{spend:,.2f} total spend)",
                "Consistent order velocity within historical norms"
            ]
            actions = [
                "Maintain standard VIP loyalty engagement",
                "Cross-sell premium accessories and new arrivals"
            ]
...
def get_churn_risk(customer_id="AA-10315"):
    ...
    # 2. Local Trained ML Model Fallback (Instant, 100% Reliable)
    local_res = _local_churn_fallback(customer_id)
    if local_res:
        return pd.DataFrame([local_res])

    return pd.DataFrame({
        "Message": ["No churn prediction found for this Customer ID."]
    })